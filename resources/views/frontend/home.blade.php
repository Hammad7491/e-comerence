...
@section('styles')
    ...
    <style>
        /* ===== THEME ===== */
        :root {
            --ink:#0f0f10; --muted:#6b7280; --rose:#c96a87; --gold:#8a6516; --card:#ffffff; --line:#ececec;
        ...
        /* GOLD RING + IMAGE INSIDE (uses <img>, works reliably on prod) */
        .wn-circle{
            position:relative;width:140px;height:140px;border-radius:999px;overflow:hidden;isolation:isolate;
            background:#f6f7fb; box-shadow:0 0 0 3px #eee, 0 0 0 5px var(--wn-gold);
        }
        .wn-photo{
        ...
        .wn-tab{font:800 11px/1 "Manrope"; text-transform:uppercase; color:#9b9b9b; text-decoration:none}
        ...
        /* ===== CARD (smaller) ===== */
        .card{
            position:relative;background:var(--card);border:1px solid #efefef;border-radius:14px;overflow:hidden;
        ...
        .card::after{content:"";position:absolute;inset:0;background:linear-gradient(120deg,transparent 40%,rgba(255,255,255,.18) 50%,transparent 60%);transform:translateX(-120%);transition:transform .7s ease;pointer-events:none}
        .card:hover{transform:translateY(-8px);box-shadow:0 18px 40px rgba(17,24,39,.15);border-color:rgba(124,17,48,.25)}
        .card:hover::after{transform:translateX(0)}

        /* MEDIA with blurred background behind the real image */
        .card .media{
            height:170px;position:relative;overflow:hidden;isolation:isolate;background:#fff;padding:0 8px;
        }
        .card .media::before{content:"";position:absolute;inset:-20%;background:var(--bg-img);background-size:cover;background-position:center;filter:blur(25px) brightness(0.4);transform:scale(1.2);z-index:1}
        .card .media::after{content:"";position:absolute;inset:0;background:radial-gradient(ellipse at center,transparent 30%,rgba(0,0,0,0.6) 100%);z-index:2}
        .card .media img{
            position:absolute;z-index:3;max-height:90%;max-width:90%;width:auto;height:auto;object-fit:contain;display:block;
            top:50%;left:50%;transform:translate(-50%,-50%);transition:transform .45s cubic-bezier(.2,.65,.2,1);
            filter:drop-shadow(0 4px 12px rgba(0,0,0,0.3));
        }
        .card:hover .media img{transform:translate(-50%,-50%) perspective(600px) translateZ(10px) scale(1.04) rotateZ(.2deg)}
        ...
        .card a.card-link{position:absolute;inset:0;z-index:10;text-indent:-9999px}
...
@php
    ...
    // Helper: force https on live to avoid mixed-content blocks
...
@section('content')
    ...
    {{-- ========== WHAT’S NEW (circles with real <img>) ========== --}}
    @php
        // Three different images (ensure filenames & CASE match on your server)
        $circleImg1 = cdn_asset('assets/images/unstiched.png');
        $circleImg2 = cdn_asset('assets/images/ready.png');
        $circleImg3 = cdn_asset('assets/images/piece.png');
        $circleFallback = 'https://via.placeholder.com/280?text=Image';
    @endphp
    ...
    <section class="fx-container wn">
            ...
            <div class="wn-circles">
                <div class="wn-chip">
                    <span class="wn-circle" aria-hidden="true">
                        <img class="wn-photo" src="{{ $circleImg1 }}?v=1" alt="Hand Embroidery Unstitched"
                             onerror="this.src='{{ $circleFallback }}'">
                    </span>
                ...
                <div class="wn-chip">
                    <span class="wn-circle" aria-hidden="true">
                        <img class="wn-photo" src="{{ $circleImg2 }}?v=1" alt="Ready To Wear"
                             onerror="this.src='{{ $circleFallback }}'">
                    </span>
                ...
                <div class="wn-chip">
                    <span class="wn-circle" aria-hidden="true">
                        <img class="wn-photo" src="{{ $circleImg3 }}?v=1" alt="Embroidered 2 Piece Suits"
                             onerror="this.src='{{ $circleFallback }}'">
                    </span>
        ...
        @php
            ...
            /**
             * Build a robust list of possible image URLs for each product (grid below).
             */
            function product_image_candidates($product) {
                $raws = [];

                if (method_exists($product, 'firstImageUrl')) $raws[] = $product->firstImageUrl();
                if (method_exists($product, 'getFirstMediaUrl')) {
                    $raws[] = $product->getFirstMediaUrl();
                    $raws[] = $product->getFirstMediaUrl('products');
                    $raws[] = $product->getFirstMediaUrl('images');
                }
                ...
                if (is_string($imgs)) {
                    ...
                    if (json_last_error() === 0 && is_array($decoded) && count($decoded)) $raws[] = $decoded[0];
                    elseif (Str::contains($imgs, ',')) { $parts = array_map('trim', explode(',', $imgs)); if (!empty($parts[0])) $raws[] = $parts[0]; }
                    else { $raws[] = $imgs; }
