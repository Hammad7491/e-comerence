...
<div class="fx-wrap">
  <div class="fx-header">
    <h3 class="fx-title">Add “What’s New” Item</h3>
  ...
  <form id="wnForm" method="POST" enctype="multipart/form-data" action="{{ route('admin.new.store') }}" class="fx-card">
    @csrf

    <div class="fx-row" style="display:grid;gap:16px">
      <div>
        <label class="fx-label">Title <span style="color:#ef4444">*</span></label>
        <input class="fx-input" type="text" name="title" required placeholder="e.g. Ready To Wear" value="{{ old('title') }}">
      </div>

      <div class="fx-file">
        <label class="fx-label">Image <small>(max 1)</small> <span id="fxCount" class="fx-counter"></span></label>
        <div class="fx-file-ui">
          <span class="fx-chip">Choose file</span><span>JPG/PNG/WEBP up to 4MB</span>
        </div>
        <input id="imageInput" type="file" name="image" accept="image/*" {{ old('image') ? '' : '' }}>
    ...
    <div class="fx-actions" style="margin-top:20px">
      <button class="fx-btn" type="submit">Save</button>
...
(function () {
  ...
  function updateCounter(has) {
    counter.textContent = `(${has ? 1 : 0}/${MAX})`;
  }
  ...
  input.addEventListener('change', e => {
    ...
    if (!file.type.startsWith('image/')) {
      msg.textContent = 'Please choose a valid image file.';
      input.value = '';
      clearPreview();
      return;
    }
